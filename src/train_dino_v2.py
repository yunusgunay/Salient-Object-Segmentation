...
def main():
    ...
    best_model_path = f"{checkpoint_dir}/dino_v2_best_model.pth"
    pr_curve_path = f"{prediction_dir}/dino_v2_pr_curve.png"
    loss_curve_path = f"{prediction_dir}/dino_v2_loss_curve.png"
    results_txt_path = f"{prediction_dir}/dino_v2_results.txt"
    ...
    train_dataset = AugmentedECSSDDataset(
        ...
        augment=True,
        normalize_mean=DINO_MEAN,
        normalize_std=DINO_STD,
    )
    ...
    model = DINOSegmenter(freeze_encoder=True).to(device)
    ...
    save_results_txt(
        ...
        pr_auc,
        results_txt_path,
        extra_info="Model: DINO v2 (BCEDice + augmentation + scheduler, frozen encoder)",
    )
