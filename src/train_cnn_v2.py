# src/train_cnn_v2.py
#
# [FINAL REPORT CHANGE] Improved CNN training script. Changes vs train_cnn.py:
#   1. Uses CNNWithSkips instead of CNNAutoencoder (skip connections + batch norm)
#   2. Uses BCEDiceLoss with pos_weight=2.0 instead of plain BCEWithLogitsLoss
#      (fixes 7-epoch dead zone caused by class imbalance)
...
def main():
    ...
    best_model_path = f"{checkpoint_dir}/cnn_improved_best_model.pth"
    pr_curve_path = f"{prediction_dir}/cnn_improved_pr_curve.png"
    loss_curve_path = f"{prediction_dir}/cnn_improved_loss_curve.png"
    results_txt_path = f"{prediction_dir}/cnn_improved_results.txt"
    ...
    # [FINAL REPORT CHANGE] pos_weight=2.0 upweights foreground pixels ~2x to counter
    # class imbalance (background dominates ~65% of ECSSD pixels).
    ...
    print("\nFinal Test Results (CNN Improved)")
    ...
    save_results_txt(
        ...
        extra_info="Model: CNN Improved (skip connections + BatchNorm + BCEDice + augmentation, 50 epochs)"
    )
